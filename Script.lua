...
if getgenv(). BenngxcHubExecuted then return end
getgenv(). BenngxcHubExecuted = true
...
--------------- Files
makefolder("BenngxcHub")
makefolder('BenngxcHub\\Anime Adventures')


local DefaultFiles = {

	['BenngxcHub \\Settings_' .. player.Name] = {
...
local Settings = HttpService:JSONDecode(readfile('BenngxcHub\\Settings_' .. player.Name)) 

local function Save (valueName, newValue)
	Settings[valueName] = newValue
	writefile('BenngxcHub\\Settings_' .. player.Name, HttpService:JSONEncode(Settings))
end

local function GetSave (valueName)
	local value = Settings[valueName]
	if value == nil then
		if DefaultFiles['BenngxcHub\\Settings_' .. player.Name][valueName] ~= nil then
			Save(valueName, DefaultFiles['BenngxcHub\\Settings_' .. player.Name][valueName])
		else
...
DiscordLabel.Text = "Discord: xwvwp6zp"
...
function getMacroUnits(macroName, textBlocks)
	...
	local macroExist = isfile('BenngxcHub\\Anime Adventures\\' .. nameMacro)
	local macroExistjson = isfile('BenngxcHub\\Anime Adventures\\' .. nameMacro .. '.json')
	...
	if macroExist or macroExistjson then
		...
		local sucess, response = pcall(function()
			local MacroAbout = HttpService:JSONDecode( readfile( 'BenngxcHub\\Anime Adventures\\' .. nameMacro) )
...
HideUHLabel.Text = 'Hide BenngxcHub'
	...
	local data = {
		...
		["embeds"] = {

			{
				...
				['color'] = 11513855,
				['footer'] = {
					['text'] = string.format("// Made by Benngxc Hub (%s)", Time), 
				},
				['fields'] = {
					{
						['name'] = 'Test Webhook',
...
for tabName, mapsList in pairs(macroMapList) do
	...
	for _, mapName in ipairs(mapsList) do
		...
		newDDLMacroMap.MouseButton1Click:Connect(function()
			...
			local MacrosList = listfiles('BenngxcHub \\Anime Adventures')
...
local function RecordMacroFunc ()
	...
	local macroExist = isfile('BenngxcHub \\Anime Adventures\\' .. MacroFile )
	local macroExistJson = isfile('BenngxcHub\\Anime Adventures\\' .. MacroFile .. '.json')

	if macroExistJson then MacroFile = 'BenngxcHub\\Anime Adventures\\' .. MacroFile .. '.json'
	elseif macroExist then MacroFile = 'BenngxcHub\\Anime Adventures\\' .. MacroFile
...
createMacroBox.FocusLost:Connect(function(enterPressed)
	...
	writefile('BenngxcHub\\Anime Adventures\\' .. createMacroBox.Text .. '.json', HttpService:JSONEncode({}))
end)
...
deleteMacroButton.MouseButton1Click:Connect(function()
	...
	local macroExist = isfile('BenngxcHub\\Anime Adventures\\' .. item)
	local macroExistjson = isfile('BenngxcHub\\Anime Adventures\\' .. item .. '.json')
	if macroExistjson then item = item .. '.json' end

	if not macroExist and not macroExistjson then return end

	delfile('BenngxcHub\\Anime Adventures\\' .. item)
...
equipMacroUnitsButton.MouseButton1Click:Connect(function()
	...
	local macroExist = isfile('BenngxcHub\\Anime Adventures\\' .. macroSelected)
	local macroExistjson = isfile('BenngxcHub\\Anime Adventures\\' .. macroSelected .. '.json')
	...
	local MacroAbout = HttpService:JSONDecode( readfile( 'BenngxcHub\\Anime Adventures\\' .. macroSelected) )
	...
	local data = {
		...
		["embeds"] = {

			{
				...
				['footer'] = {
					['text'] = string.format("// Made by Benngxc Hub (%s)", os.date("%X")), 
				},
				['fields'] = {
					{
						['name'] = "Player Stats",
...
task.spawn(function() 
	pcall(function()
		...
		if queue_on_teleport then
			local BenngxcHub = 'loadstring(game:HttpGet("https://raw.githubusercontent.com/ngxc42/Benngxc/main/benngxchub"))();'
			queue_on_teleport(BenngxcHub)
		end
